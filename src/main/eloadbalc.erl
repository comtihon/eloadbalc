...
%% API
-export([get_less_loaded/0, add_node/3]).
...
-spec add_node(atom(), integer(), integer() | atom()) -> ok.
add_node(Node, MaxValue, UpdateTime) ->
  eb_logic_worker:add_node({Node, MaxValue, UpdateTime}).
